...
class Playwright_DocumentLoaders implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        async function playwrightLoader(url: string, pageIndex: number, lastModified: number | null = null): Promise<any> {
            try {
                let docs = []
                ...
                if (isDebug) options.logger.error(`error in PlaywrightWebBaseLoader: ${err.message}, on page: ${url}`)
            }
        }
        ...
        if (relativeLinksMethod) {
            ...
            for (const [index, page] of pages.entries()) {
                const loadedDocs = await playwrightLoader(page, index, pagesLastMod[page])
